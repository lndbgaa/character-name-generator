...
export const updateProfileSchema = Joi.object({
  username: Joi.string().label("Username").trim().min(3).max(20).pattern(usernameRegex).optional().messages({
  ...
  firstName: Joi.string().label("First Name").trim().min(2).max(50).pattern(nameRegex).optional().messages({
  ...
  lastName: Joi.string().label("Last Name").trim().min(2).max(100).pattern(nameRegex).optional().messages({
  ...
  password: Joi.string().label("Password").trim().min(8).max(100).pattern(passwordRegex).optional().messages({
